...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
    ...
    // b) wait for switch to be pressed
		WaitForASHigh(); // When the switch SW1 is pressed (AS=0)
...
In this lab you will create seven subroutines. Solving a complex problem by breaking it into simpler subtasks is an effective approach to design. 
...
When the switch SW1 is pressed (AS=0) it means the atria has begun to contract. When the switch SW1 is released, it means the atria have completed contraction. 
...
In a real pacemaker, electrodes (wires) are attached to the heart, and electrical pulses are generated that stimulate the heart, causing it to beat. 
...
// write this function
	while(1){
		if((GPIO_PORTF_DATA_R &0x10) == 0x10) { // AS input is connected to PF4
			return;
		}
	}
...
// write this function
	while(1){
		if((GPIO_PORTF_DATA_R &0x10) == 0x00) { // When the switch SW1 is pressed (AS=0)
			return;
		}
	}
}
...
void SetVT(void){
// write this function
	GPIO_PORTF_DATA_R |= 0x02; // set bit 1
	// VT (PF1) so 2^1 = 2
}
...
void ClearVT(void){
// write this function
	GPIO_PORTF_DATA_R &= ~0x02; // clear bit 1
}
...
void SetReady(void){
// write this function
	GPIO_PORTF_DATA_R |= 0x08; // set bit 3
	// Ready (PF3) so 2^3 = 8
}
...
void ClearReady(void){
// write this function
	GPIO_PORTF_DATA_R &= ~0x08; // clear bit 3
}
