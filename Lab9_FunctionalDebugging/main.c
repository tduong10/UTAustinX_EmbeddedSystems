...
void Delay(void){
  ...
  //time = 160000; // 0.1sec. original time.
	time = 160000/2; // If either PF4 or PF0 switches is pressed, the output is inverted every 50 ms
  while(time){
   time--;
  }
}
...
int main(void){  
  ...
  while(1){
		...
		unsigned long SW1 = GPIO_PORTF_DATA_R &0x10;
		unsigned long SW2 = GPIO_PORTF_DATA_R &0x01; // PF4 = 0x10, PF0 = 0x01
		if (SW1 == 0x00 || SW2 == 0x00) { 		// make the LED flash if either switch SW1 or SW2 are pressed (this means either PF4 or PF0 is 0)
			Led = GPIO_PORTF_DATA_R;   // read previous
			Led = Led^0x02;            // toggle red LED. This means toggle if just SW1 is pressed, just SW2 is pressed or both switches are pressed.
			GPIO_PORTF_DATA_R = Led;   // output      
			
			if(i<50){
				now = NVIC_ST_CURRENT_R;
				Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
				Data[i] = GPIO_PORTF_DATA_R &0x13; // If your system causes a change in PF1, record PortF bits 4,1,0. PF4 = 0x10, PF0 = 0x01, PF1 0x02
				last = now;
				i++;
			}
		}
