...
// as part of Lab 11 implement this function	
	//The decimal digits 0 to 9 are encoded in ASCII as 0x30 to 0x39. So, to convert a single ASCII digit to a decimal number, we simply subtract 0x30. 
	//Unsigned decimal to ASCII string conversion is + 0x30
	int i, result;
	if (n > 9999)
	{
		for (i = 0; i < 4; i++)
	...
	else
	{
		...
		for (j = 3; j >= 0; j--)
		{
			if (n > 0 || j == 3) // if n=0 then this won't go through 
			{
				result = n % 10; // e.g. 1234 = 4, 123 = 3, 12 = 2, 
				String[j] = (int)result + 0x30;
			}
			else
			{
				String[j] = ' ';
	...
	String[4] = ' '; // empty space before null
	// e.g. 4 is in index 3
